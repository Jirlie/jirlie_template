...
def get_context(context):
    context.blog2 = frappe.get_doc("Blog Post", frappe.form_dict.name2)
    context.blogger2 = frappe.get_doc("Blogger", context.blog2.blogger)
    context.blog_category2 = frappe.get_doc("Blog Category", context.blog2.blog_category)
    context.category2 = frappe.db.sql("""SELECT * from `tabBlog Post` where `blog_category`= '{{frappe.form_dict.blog_category2}}' """, as_dict=True)
    return context
